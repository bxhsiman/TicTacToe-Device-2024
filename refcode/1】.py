...
def detect_and_return_board(frame):
    # 调用棋盘检测函数
    ...
    return rect, chess_positions, board


def get_board(camera_index=1):
    # 打开摄像头
    ...
    # 初始化存储历史棋盘状态的数组
    history_boards = np.zeros((3, 3, 9), dtype=int)
    # 循环进行多次读取和检测
    for i in range(9):
        ret, frame = cam.read()
        if not ret:
            ...
            continue
        # 检测棋盘和棋子
        _, _, board = detect_and_return_board(frame)

        if board is not None:
            # 将当前棋盘状态存储到历史数组中
            history_boards[:, :, i] = np.array(board)

    # 计算 board_true
    board_true = np.zeros((3, 3), dtype=int)

    # 遍历每个格子，统计出现次数最多的棋子状态
    for i in range(3):
        for j in range(3):
            values, counts = np.unique(history_boards[i, j, :], return_counts=True)
            if len(values) == 0:
                continue
            max_count_idx = np.argmax(counts)
            most_frequent_value = values[max_count_idx]
            if counts[max_count_idx] >= 2:
                board_true[i, j] = most_frequent_value
            else:
                board_true[i, j] = 0
    cam.release()
    return  board_true
...
if not cam.isOpened():
    ...
    board_true=get_board(camera_index=1)
    # 打开摄像头
    cam = cv2.VideoCapture(1)  # 使用摄像头1，可以根据实际情况修改
    print(board_true)
    while True:
